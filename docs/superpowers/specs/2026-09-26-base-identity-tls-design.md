...
2. TLS：**仅节点↔节点**（双向 mTLS + 自签证书 + 指纹固定 + 首次配对由人确认）；**客户端↔节点走明文 HTTP**（退路 F1，见 §6.1）。
...
- **不做客户端↔节点 TLS**（退路 F1，2026-09-26 定案）：客户端走明文 HTTP；见 §6.1。
...
**两条链路分开定**（总纲 §12.2）：节点↔节点 = 双向 mTLS + 指纹固定（§6.1 保留「自签 + 配对码」机制）；客户端↔节点 = **明文 HTTP**（§6.1 F1 结论）。

### 6.1 结论 F1：客户端明文 + 节点间自签指纹固定

**F1（2026-09-26 定案，取代原「客户端↔节点也走 TLS」）：**

- **客户端↔节点：明文 HTTP。** ① 类内容本就匿名全网公开（总纲 §3 铁律 2），TLS 的机密性无收益；完整性 / 认证由签名头（§3.1）与 manifest 验签承担。代价：明文链路上被动旁观者可读 ① 类明文与请求路径、写请求正文同样可读；截断可阻断（可用性不保），但**不可伪造内容、不可冒用身份**。
- **节点↔节点：双向 mTLS + 指纹固定**。节点首启若无 `data/tls/` 证书 → 自签生成（ECDSA P-256）→ 计算证书 SHA-256 指纹 → 生成**配对码**：取指纹前 10 字节 → Base32 → 16 字符，按 4-4-4-4 分组显示（如 `ABCD-EFGH-IJKL-MNOP`）；同时输出完整指纹 hex 供复制。
- 对端清单由人核对配对码后写入 `BASE_PEERS`（`[{"url":"https://...","tls_fingerprint":"<hex>"}]`）；两侧指纹互为白名单，非白名单对端在 mTLS 握手即被拒（`tls: bad certificate`）。**不提供「忽略」开关**。

**`-tls-cert=off` 的语义边界（F1 落地时暴露并已修）**：只关**主监听**加密，**不关闭节点自身身份**。启用对端监听时，节点仍从默认路径 `data/tls/node.crt|key` 生成/加载证书；若把 off 当成「没有证书」，`serve` / `scrub` / `peer-sync` 会直接报「证书与私钥路径都不能为空」。

> **落地状态（2026-09-26 定案回填）**：Task 1 的「客户端自签 TLS 真机 spike」**未执行**——验证需真机与具体 ROM。改为按成本直接定案 F1（理由与代价如上），因此 §8 的 `nodes` 表与客户端配对交互**不做**。已落地的部分：节点侧证书生成 / 配对码 / 指纹固定与拒连、节点↔节点双向 mTLS + `X-Base-Node-Key`、浏览页与启动日志展示配对码。生产实况：主节点 TLS `:443`（对端 `:8081`），缓存节点明文 `:80`（对端 `:8082`）；两侧反熵收敛、`:8082` 拒绝无证书客户端、缓存节点指纹不变。

### 6.2 换证书

节点执行 `rotate-cert` → 生成新自签证书 → 浏览页与启动日志显示**新配对码** → 所有**对端**需重新写入指纹。旧指纹不会自动接受。

### 6.3 节点↔节点

- 节点清单由配置提供：`BASE_PEERS` = JSON 数组 `[{"url":"https://...","tls_fingerprint":"<hex>"}]`（JSON 里含双引号，写进 systemd 单元时作为 `ExecStart` 参数最稳，见册子 #5 §0.2 第 9 条）。
- **硬校验**：`-peer-addr` 非空 ⇒ `-peers` 指纹白名单必须非空（否则启动报错）。对端监听默认**不启用**。
...
| ~~`nodes(node_id, url, tls_fingerprint, pairing_verified_at, last_seen_at)`~~ | 已配对节点与固定指纹 —— **F1 下不做**：客户端走明文 HTTP，无客户端侧证书可固定（§6.1）。客户端只需存节点根地址（`node_base_url`）+ 签发方公钥（`pubkey_hex`）；节点指纹只存在于**节点侧**的 `BASE_PEERS` 白名单 |
...
| 9 | 对端 TLS 指纹 | **仅节点↔节点**：指纹不匹配的对端连接被拒；配对码核对写入白名单后连接成功；无「忽略」路径。客户端链路走明文 HTTP，另断言公开读接口明文下仍 200 |
...
5. **对端**指纹必须由人核对配对码后写入白名单；不得提供「忽略指纹不匹配」的开关。客户端链路是明文，不得把「明文链路」当作可信来源。
...
| TLS | **仅节点↔节点**：自签 + 证书指纹固定 + 配对码（指纹前 10 字节 Base32，4-4-4-4）人工核对后写入 `BASE_PEERS` 白名单；无忽略开关。**客户端↔节点走明文 HTTP**（F1，2026-09-26 定案） |
...
| 不做 | L4b、L4c、① 类 E2E、**客户端↔节点 TLS（F1）**、JWT、读取鉴权、密钥轮换、**§8 的 `nodes` 表与客户端配对交互** |
...
6. TLS：自签生成 + 配对码输出 + **节点侧**指纹白名单拒连（客户端侧不做固定，F1）
